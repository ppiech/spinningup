...
class ObservationsActionsAndGoalsBuffer:
    ...
    def __init__(self, obs_dim, goal_octaves, act_dim, size):
        ...
        self.goals_num_buf = np.zeros(size , dtype=np.float32)
    ...
    def store(self, obs, new_obs, goal_num, goal_bin, act):
        ...
        self.goals_num_buf[self.ptr] = goal_num
        self.goals_bin_buf[self.ptr] = goal_bin
    ...
    def all_get(self):
        ...
        goals_num_buf = allgather(self.goals_num_buf)
        goals_bin_buf = allgather(self.goals_bin_buf)
        act_buf = allgather(self.act_buf)

        # if self.ptr == self.max_size:
        to_return = [obs_buf, new_obs_buf, goals_num_buf, goals_bin_buf, act_buf]
        # else:
        #     to_return = [obs_buf[:self.ptr], new_obs_buf[:self.ptr], goals_num_buf[:self.ptr],
        #                  goals_bin_buf[:self.ptr], act_buf[:self.ptr]]
    ...
    def append(self, buffer):
        ...
        new_obs, new_new_obs, new_goal_nums, new_goal_bufs, new_act = buffer.all_get()
        buf_len = buffer.ptr
        ...
        for mpi_proc_num in range(len(new_obs)):
            for i in range(buf_len):
                ...
                self.goals_num_buf[insert_at] = new_goal_nums[mpi_proc_num][i]
                self.goals_bin_buf[insert_at] = new_goal_bufs[mpi_proc_num][i]
...
def goaly(
    ...
    goals_bin_ph = tf.placeholder(dtype=tf.float32, shape=(None, goal_octaves), name="goals_bin_ph")
    goals_num_ph = tf.placeholder(dtype=tf.int32, shape=(None, ), name="goals_num_ph")
    ...
    with tf.variable_scope('goals'):
        goals_pi, goals_logp, goals_logp_pi, goals_v = actor_critic(
            x_ph, None, goals_num_ph, action_space=Discrete(num_goals), **ac_kwargs)

    with tf.variable_scope('actions'):
        actions_pi, actions_logp, actions_logp_pi, actions_v = actor_critic(
            x_ph, goals_bin_ph, actions_ph, action_space=env.action_space, **ac_kwargs)
    ...
    a_inverse, a_predicted, goals_predicted_logits, goals_predicted = \
        core.inverse_model(env, x_ph, x_next_ph, actions_ph, goals_bin_ph, goal_octaves, split_action_and_goal_models, **inverse_kwargs)
    ...
    inverse_goal_diff = tf.reduce_mean(tf.abs(tf.cast(goals_bin_ph - goals_predicted_logits, tf.float32)), axis=-1, name='inverse_goal_diff')
    ...
    inverse_goal_loss = tf.reduce_mean((tf.cast(goals_bin_ph - goals_predicted_logits, tf.float32)**2), name='inverse_goal_loss')
    ...
    inverse_goal_error = tf.reduce_mean(tf.abs(tf.cast(goals_predicted - goals_bin_ph, tf.float32)))
    ...
    # Setup model saving
    logger.setup_tf_saver(sess, inputs={'x': x_ph, 'goals_bin_ph': goals_bin_ph, 'goals_num_ph': goals_num_ph }, \
    ...
    def update():
        ...
        inputs = {k:v for k,v in zip([x_ph, x_next_ph, goals_num_ph, goals_bin_ph, actions_ph], trajectory_buf.get())}
    ...
    goal_num, stability, action_error, goal_error, forward_prediction_error, goal_discount, discounted_stability, goal_predicted_v = 0, 0, 0, 0, 0, 0, 0, 0

    ep_obs = [[]]
    episode = 0

    def store_training_step(observations, new_observations, goal_num, goal_bin, goals_step_reward, actions,
                            actions_reward_v, reward, goals_v_t, goals_logp_t, stability, actions_v_t,
                            forward_prediction_error):
        trajectory_buf.store(observations, new_observations, goal_num, goal_bin, actions)
        ...
        logger.store(Goal=goal_num)
        # logger.storeOne("Goal{}Reward".format(goal), reward)
        logger.store(GoalPathLen=actions_ppo_buf.path_len())

    def log_trace_step(epoch, episode, observations, actions, goal_num, reward, goals_v_t, \
        ...
        if episode % trace_freq == 0:
            ...
            traces_logger.log_tabular('Goal', goal_num)
    ...
    def calculate_stability(observations, new_observations, actions, goal_num, goal_bin, num_goals):
        action_error, forward_prediction_error, goals_predicted_bin_v, x_pred_v = \
            sess.run([inverse_action_error, forward_error, goals_predicted, x_pred],
        ...
        goal_error = abs((goal_num - goal_predicted_num) / num_goals)
        ...
        return stability, action_error, goal_error, forward_prediction_error, goal_predicted_num

    def handle_episode_termination(episode, goal_num, goal_bin, prev_goal_num, observations, reward, done, ep_ret, ep_len, stability):
        ...
        if terminal or (t==local_steps_per_epoch-1):
            ...
            if done:
                last_actions_val = actions_reward(reward, stability) + reward
                last_goals_val = goals_step_reward(reward, stability) + reward
            else:
                last_actions_val, last_goals_val = \
                    sess.run([actions_v, goals_v], feed_dict={x_ph: observations.reshape(1,-1), goals_bin_ph: [goal_bin]})
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            goal_num = goal[0]
            goal_bin = core.goal_num_to_bin(goal_num, goal_octaves)

            actions, actions_v_t, actions_logp_t = \
                sess.run([actions_pi, actions_v, actions_logp_pi],
                         feed_dict={x_ph: observations.reshape(1,-1), goals_bin_ph: [goal_bin] })
            ...
            store_training_step(observations, new_observations, goal_num, goal_bin, goals_step_reward_v, actions,
            ...
            stability, action_error, goal_error, forward_prediction_error, goal_predicted_v = \
                calculate_stability(observations, new_observations, actions, goal_num, goal_bin, num_goals)

            log_trace_step(epoch, episode, observations, actions, goal_num, reward,
            ...
            episode, observations, reward, done, ep_ret, ep_len = \
                handle_episode_termination(episode, goal_num, goal_bin, prev_goal_num, observations, reward, done, ep_ret, ep_len, stability)
